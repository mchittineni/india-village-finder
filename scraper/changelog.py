...
def git_show(rel_path: str) -> str | None:
    try:
        out = subprocess.run(["git", "show", f"HEAD:{rel_path}"],
...
def state_section(name: str, slug: str) -> tuple[str, bool]:
    ...
    new_c = counts((ROOT / meta_rel).read_text(encoding="utf-8")) if (ROOT / meta_rel).exists() else {}
    ...
    lines += [f"**{len(added)} added · {len(removed)} removed · {len(changed)} reclassified/renamed**", ""]
...
def main():
    ...
    parts += ["", "---", "*Generated by `scraper/changelog.py`. Source: "
              "[LGD](https://lgdirectory.gov.in) via "
